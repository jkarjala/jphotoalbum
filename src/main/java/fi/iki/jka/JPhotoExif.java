...
public class JPhotoExif implements Serializable {
    ...
    public JPhotoExif(ExifSubIFDDirectory dir) {
        if (dir==null)
            return;
        
        try {
            if (dir.containsTag(ExifSubIFDDirectory.TAG_EXIF_IMAGE_WIDTH))
                width = dir.getInt(ExifSubIFDDirectory.TAG_EXIF_IMAGE_WIDTH);
            
            if (dir.containsTag(ExifSubIFDDirectory.TAG_EXIF_IMAGE_HEIGHT))
                height = dir.getInt(ExifSubIFDDirectory.TAG_EXIF_IMAGE_HEIGHT);
            
            aperture = dir.getDescription(ExifSubIFDDirectory.TAG_APERTURE);
            if (aperture==null)
                aperture = getRational(dir, ExifSubIFDDirectory.TAG_FNUMBER);
            if (aperture==null)
                aperture = "";
            ...
            exposureTime = getRational(dir, ExifSubIFDDirectory.TAG_EXPOSURE_TIME);
            if (exposureTime==null)
                exposureTime = "";
            if (exposureTime.length()>0 && exposureTime.indexOf('s')<0)
                exposureTime += "s";

            exposureBias = getRational(dir, ExifSubIFDDirectory.TAG_EXPOSURE_BIAS);
            if (exposureBias==null || exposureBias.equals("0"))
                exposureBias = "";
            
            date = dir.getDescription(ExifSubIFDDirectory.TAG_DATETIME_ORIGINAL);
            if (date==null) {
            	date = dir.getDescription(ExifSubIFDDirectory.TAG_DATETIME_DIGITIZED);
            	if (date==null)
            		date = "";
            }
    ...
    protected String getRational(ExifSubIFDDirectory dir, int id) throws Exception {
