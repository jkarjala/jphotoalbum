 ...
 *
 * @version $Id: JPhoto.java,v 1.14 2013/07/20 08:00:23 jkarjala Exp $
 */
...
import static com.drew.metadata.exif.ExifIFD0Directory.TAG_IMAGE_DESCRIPTION;
...
public class JPhoto extends Observable implements Transferable, Serializable {
    ...
    transient protected Metadata metadata = null;
    transient protected ExifSubIFDDirectory directory = null;
...
//                imageReader.dispose();
    ...
    public IptcDirectory getIptcDirectory() {
        IptcDirectory iptcDirectory = null;
        if (iptcDirectory==null && getOriginalFile()!=null) {
            try {
                ...
                setStatus(e.toString());
                JPhotoStatus.showStatus(null, getOriginalFile()+": cannot read iptc "+e);
            }
        }
            ...
            if (directory!=null) {
                ...
                if (getText()==null || getText().equals("")) { // initialise image text from IPTC if nothing else available
                    ...
                    try {
                        ...
                        if (bytes!=null) {
                            // Lightroom 5 writes UTF8, other programs may use other charsets, no way to know... 
                            String caption = new String(bytes, "utf8"); 
                            setText(caption);
                        }
                    }
                    catch (Exception e) {
                        System.err.println("Caption parse error:"+e);
                    }
                }
            }
            else
    ...
    public void setWebSize() {
        ...
        if (fullWidth<0 || fullHeight<0) {
            ...
            ExifSubIFDDirectory exif = getExifDirectory();
            if (exif!=null && exif.containsTag(ExifSubIFDDirectory.TAG_EXIF_IMAGE_WIDTH)) {
                try {
                    fullWidth = exif.getInt(ExifSubIFDDirectory.TAG_EXIF_IMAGE_WIDTH);
                    fullHeight = exif.getInt(ExifSubIFDDirectory.TAG_EXIF_IMAGE_HEIGHT);
                } catch (Exception e) {
                    System.out.println("setWebSize from exif failed"+e);
                }
            }
            else {
                System.out.println("No EXIF_IMAGE_WIDTH?");
            }
        } 
        else {
